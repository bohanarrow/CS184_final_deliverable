...
_global double lightX 	= 0;
_global double lightY 	= 0;

_global int saveTex     = 0;
_global int fireLen = 0;

_global int antiAlias = 0;
...
_global particle_t* old_buffer = {0};

_global int* lights = {0};
...
gs_result app_init()
{
    ...
    lights = gs_malloc(g_texture_width * g_texture_height * sizeof(int));

	// Construct texture buffer data
	g_texture_buffer = gs_malloc( g_texture_width * g_texture_height * sizeof(color_t) );

	old_buffer = gs_malloc( g_texture_width * g_texture_height * sizeof(particle_t) );
...
void update_input()
{
    ...
    if ( platform->key_pressed( gs_keycode_a ) ) {
        if(in_bounds((lightX - 10), lightY)) {
            lightX -= 10;
        }
    }
    if ( platform->key_pressed( gs_keycode_w ) ) {
        if(in_bounds(lightX, (lightY - 10))) {
            lightY -= 10;
        }
    }
    ...
    if ( platform->key_pressed( gs_keycode_d ) ) {
        if(in_bounds((lightX + 10), lightY)) {
            lightX += 10;
        }
    }
    if ( platform->key_pressed( gs_keycode_j ) ) {
        if(antiAlias){
        	antiAlias = 0;
        }
        else{
    ...
    if ( platform->key_pressed( gs_keycode_r ) ) {
        int loop;
        for(loop = 0; loop < g_texture_width * g_texture_height; loop++){
            old_buffer[loop] = g_world_particle_data[loop];
        }
        saveTex = 1;
    }
...
double getCount(x,y){
	u8 new_mat = get_particle_at(x,y).id;
	if(new_mat == mat_id_water){
		return .01;
	}
	if(new_mat == mat_id_sand){
		return .2;
	}
	if(new_mat == mat_id_oil){
		return .2;
	}
	if(new_mat == mat_id_smoke){
		return .1;
	}
	if(new_mat == mat_id_wood){
		return .15;
	}
	if(new_mat == mat_id_salt){
		return .2;
	}
	if(new_mat == mat_id_gunpowder){
		return .2;
	}
	if(new_mat == mat_id_steam){
		return .05;
	}
	if(new_mat == mat_id_stone){
		return .2;
	}
	if(new_mat == mat_id_lava){
		return .075;
	}
	if(new_mat == mat_id_acid){
		return .075;
	}
	return 0;
}

double surroundCount(double x, double y){
		// Top
	double count = 0;
	double total = 0;
...
double sampleLight(u32 x, u32 y, double liX, double liY){
    double yInt = (double)y;
    double xInt = (double)x;
    double dist = abs(xInt - liX) + abs(yInt-liY);
    double run = xInt - liX;
    double rise = yInt - liY;
    double bigX;
    double bigY;
    double dx = 1;
    double dy = 1;
    ...
    double count = 1;
    double tempY = yInt;
    double holderX = tempX;
    double holderY = tempY;
    int stop = 0;
    double total = 1;
    while ((holderX!=liX || holderY!=liY) && count < 5.5) {
    	holderX = tempX+ dx;
    	tempX += bigX;
    	holderY = tempY + dy;
	    tempY += bigY;
    	while(1){
	        while (1) {
	        	//count += surroundCount(holderX,holderY);
	            if(in_bounds(holderX, holderY) && !is_empty(holderX, holderY)){
	            	if(antiAlias){
	            		count += surroundCount(holderX, holderY);
	            	}
	            	else{
	            		count += getCount(holderX, holderY);
	            	}
	            	
	            }
	            total += .2;
	            if(holderY == liY || holderY == tempY || count >= 5.5){
	            	if(holderY == liY){
		    			bigY = 0;
		    			dy = 0;
		    			tempX = holderY;
	    	...
	    	if(holderX == liX || holderX == tempX || count >= 5.5){
	    		if(holderX == liX){
	    			bigX = 0;
	    			dx = 0;
	    			tempX = holderX;
    ...
    int loop;
    //count = (count/total * dist)*.2 + 1;
    if(fireLen>0){
        for (loop = 0; loop < fireLen; loop += 1) {
            if(random_val(0,20) == 0){
                continue;
            }
            double lX = lights[loop] % g_texture_width;
            double lY = floor(lights[loop] / g_texture_width);
            double dist = abs(xInt - lX) + abs(yInt-lY);
            run = xInt - lX;
		    rise = yInt - lY;
		    bigX;
		    bigY;
		    ...
		    holderX = tempX;
		    holderY = tempY;
            double sub = .5 - dist/600.0;
            while (holderX!=lX || holderY!=lY) {
		    	holderX = tempX+ dx;
		    	tempX += bigX;
		    	holderY = tempY + dy;
			    tempY += bigY;
		    	while(1){
			        while (1) {
			            ...
			            if(in_bounds(holderX, holderY) && !is_empty(holderX, holderY)){
			                sub -= .1;
	                        u8 new_mat = get_particle_at(holderX, holderY).id;
	                        if (new_mat == mat_id_water){
	                            sub += .09;
	                        }
	                        if (new_mat == mat_id_fire || new_mat == mat_id_ember){
	                            sub += .1;
	                        }
			            }
			            if(holderY == lY || holderY == tempY){
			            	if(holderY == lY){
				    			bigY = 0;
				    			dy = 0;
				    			tempX = holderY;
...
void update_particle_sim()
{
	...
	const f32 dt = .03;//platform->time.delta;
...
//            particle_t* p = &g_world_particle_data[compute_idx( x, y )];
//            if(!is_empty(x,y)){
//                int temp = x-1;
//                int flag = 1;
//                while(temp>0){
//                    if(!is_empty(temp,y)){
//                        p->color.r = p->orig_color.r /2;
//                        p->color.g = p->orig_color.g /2;
//                        p->color.b = p->orig_color.b /2;
//                        flag = 0;
//                    }
//                    temp -= 1;
//                }
...
//            }
    ...
    for ( u32 y = g_texture_height - 1; y > 0; --y ) {
        for (u32 x = ran ? 0 : g_texture_width - 1; ran ? x < g_texture_width : x > 0; ran ? ++x : --x) {
            u32 read_idx = compute_idx(x, y);
            if(g_world_particle_data[read_idx].id == mat_id_fire && random_val( 0, 40 ) == 0){
                lights[fireLen] = read_idx;
                fireLen += 1;
            }
        }
    }
    for ( u32 y = g_texture_height - 1; y > 0; --y ) {
        for (u32 x = ran ? 0 : g_texture_width - 1; ran ? x < g_texture_width : x > 0; ran ? ++x : --x) {
            ...
            // Update particle's lifetime (I guess just use frames)? Or should I have sublife?
            //g_world_particle_data[read_idx].life_time += 1.f * dt;
            double count  = sampleLight(x,y,lightX, lightY);
            if(antiAlias){
            	count += sampleLight(lightX,lightY,x, y);
            	count = count/2.0;
            }
            if (count > 1) {
                p->color.r = p->orig_color.r / count;
                p->color.g = p->orig_color.g / count;
                p->color.b = p->orig_color.b / count;
            }
            else{
                p->color = p->orig_color;
            }
...
void draw_glyph_at( font_t* f, color_t* buffer, s32 x, s32 y, char c, color_t col ) 
{
	...
	for ( s32 h = 0; h < g.height; ++h ) 
	{
		for ( s32 w = 0; w < g.width; ++w ) 
		{
			...
			color_t c = {
				255,//font_data[ ( _h * f->width + _w ) * f->num_comps + 0 ],
				255,//font_data[ ( _h * f->width + _w ) * f->num_comps + 1 ],
				255,//font_data[ ( _h * f->width + _w ) * f->num_comps + 2 ],
				a
			};
...
// Simple falling, changing the velocity here ruins everything. I need to redo this entire simulation.
	    ...
	    if(saveTex && old_buffer[read_idx].id == mat_id_empty){
...
void update_sand( u32 x, u32 y )
{
	f32 dt = .03;//gs_engine_instance()->ctx.platform->time.delta;
